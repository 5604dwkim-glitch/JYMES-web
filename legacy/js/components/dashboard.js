...
import { store, CAR_MODELS, CAR_MODEL_PARTS, MANUFACTURERS } from '../store.js';
...
function updateSubPartChartAndTable(container) {
  ...
  targetCarCodes.forEach(carCode => {
    ...
    parts.forEach(part => {
      ...
      const totalActual = partReports.reduce((sum, r) => sum + (r.actualQty || 0), 0);
      const totalDefect = partReports.reduce((sum, r) => sum + (r.defectQty || 0), 0);
  ...
  mainPartChart = new Chart(canvas, {
    ...
    options: {
      ...
      scales: {
        ...
        y1: { type: 'linear', position: 'right', min: 0, max: 4 }
      }
    }
  });
}
