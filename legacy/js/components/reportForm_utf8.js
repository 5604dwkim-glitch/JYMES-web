...
import { store, MANUFACTURERS, CAR_MODELS, CAR_MODEL_PARTS, DEFAULT_LEADER_ITEMS, DEFAULT_ATTENDANCE, DOWNTIME_REASONS } from '../store.js';
import { i18n, STRING_TO_KEY_MAP } from '../i18n.js';
...
export function renderReportForm(container, editingReportId = null) {
  ...
  btnSelectLeaderForm.addEventListener('click', () => {
    if (!isLeaderRole) {
      window.showToast(`⚠️ '작업일보(반장)' 양식은 직급이 [반장]인 사용자만 선택할 수 있습니다. (현재 직급: ${userPosition})`, 'warning');
      return;
    }
    ...
    try {
      ...
      if (existingData) {
        ...
        if (targetStatus === '임시저장') {
          window.showToast('📁 작업일보가 중간 저장되었습니다.', 'info');
        } else {
          window.showToast('✅ 작업일보가 등록 완료되었습니다.', 'success');
        }
      } else {
        store.addReport(reportData);
        if (targetStatus === '임시저장') {
          window.showToast('📁 장수미 반장 작업일보 중간 저장 성공!', 'info');
        } else {
          window.showToast('✅ 장수미 반장 작업일보 등록 완료 성공!', 'success');
        }
      }
      ...
      if (window.appInstance) {
        ...
        if (userRoleInfo && userRoleInfo.role === 'worker') {
          window.appInstance.switchTab('drafts');
        } else {
          window.appInstance.switchTab('reports');
        }
      }
    } catch (err) {
      console.error('Error saving leader report:', err);
      window.showToast(`⚠️ 저장 중 오류가 발생했습니다: ${err.message}`, 'error');
    }
  };
...
function setupStandardMobileEvents(container, existingData, defaultMakerName, defaultCarCode, loggedInWorkerName = '', materialLots = {}) {
  ...
  function getKmKxClipQtyHTML(ed) {
  ...
  function getDtCrewPostQtyHTML(ed) {
  ...
  function getKmKxPostQtyHTML(ed) {
    const q = ed && ed.dtCrewPostQty ? ed.dtCrewPostQty : {};
    const prod = q.prod || {};
    const ext = q.ext || {};
    const joint = q.joint || {};
  ...
  function getDtCrewJointQtyHTML(ed, defaultPlan = 300, isFixedLhRh = false) {
  ...
  function getKmKxJointQtyHTML(ed, defaultPlan = 300) {
    const q = ed && ed.dtCrewJointQty ? ed.dtCrewJointQty : {};
    const q1 = q.tbl1 || {};
    const q2 = q.tbl2 || {};
    const defs1 = q1.defects || {};
  ...
  function getDtCrewEndJointQtyHTML(ed) {
    const q = ed && ed.dtCrewJointQty ? ed.dtCrewJointQty : {};
    const q1 = q.tbl1 || {};
    const defs1 = q1.defects || {};
  ...
  function getDtCrewPrepQtyHTML(ed) {
  ...
  function getStandardQtyHTML(ed) {
  ...
  // Form 4001 생산실적
  function getForm4001QtyHTML(ed) {
    const q = ed && ed.qtyTable ? ed.qtyTable : {};
    return `
      <div class="card" style="padding: 16px; margin-bottom: 16px;">
        <label style="font-size: 14px; font-weight: 700; color: var(--accent-blue); margin-bottom: 8px; display: block;">
          ...
          <table id="jg1QtyTable" style="width: 100%; border-collapse: collapse; border: 2px solid #000; text-align: center; font-size: 11px; background: #fff; font-family: 'Noto Sans KR', sans-serif;">
            <thead>
              <tr style="background: #fffde7; font-weight: 700;">
                ...
                <th style="border: 1px solid #000; padding: 8px 4px; font-size: 12px; color: #000; width: 70%;">FRT LH</th>
              </tr>
            </thead>
            <tbody>
              <tr>
              ...
              <tr>
                <td style="border: 1px solid #000; padding: 6px; background: #fffde7; font-weight: 700; color: #000;">기타 (The others)</td>
                <td style="border: 1px solid #000; padding: 2px;"><input type="number" id="def_proc_oth_FL" class="form-control qty-calc-input" style="width: 100%; border: none; text-align: center; font-size: 11px; padding: 4px;" value="${q.proc_oth_FL ?? ''}" placeholder="0" /></td>
              </tr>
              <tr style="background: #fffde7; font-weight: 700;">
  ...
  function calcForm4001QtySummary() {
    const table = container.querySelector('#jg1QtyTable');
    if (!table) return;
    ...
    const plan = Number(table.querySelector('#qty_plan_FL')?.value) || 0;
    const act = Number(table.querySelector('#qty_act_FL')?.value) || 0;
    totalPlan += plan;
    totalAct += act;

    const defs = [
      Number(table.querySelector('#def_ext_scorch_FL')?.value) || 0,
      Number(table.querySelector('#def_ext_scratch_FL')?.value) || 0,
      Number(table.querySelector('#def_ext_flock_FL')?.value) || 0,
      Number(table.querySelector('#def_ext_contam_FL')?.value) || 0,
      Number(table.querySelector('#def_proc_len_FL')?.value) || 0,
      Number(table.querySelector('#def_proc_cut_FL')?.value) || 0,
      Number(table.querySelector('#def_proc_oth_FL')?.value) || 0
    ];

    const posDefectSum = defs.reduce((a, b) => a + b, 0);
    overallDefect += posDefectSum;

    const sumElem = table.querySelector('#def_sum_FL');
    if (sumElem) sumElem.textContent = posDefectSum;
...
${renderDtRow4('종')}
  ...
  function getKmKxInspQtyHTML(ed) {
  ...
  function getInspQtyHTML(ed, colCount = 4, autoInspectQty = false) {
    const inspData = ed && ed.inspQtyTable ? ed.inspQtyTable : {};
  ...
  function renderQtySection() {
    ...
    switch (formCode) {
      // ── 현대/기아 소재준비 (#4001) ──
      case 4001:
        qtySection.innerHTML = getForm4001QtyHTML(existingData);
        qtySection.addEventListener('input', calcForm4001QtySummary);
        calcForm4001QtySummary();
        break;

      // ── 클립머신 (#2001, #2011) ──
      case 2001:
      case 2011:
        qtySection.innerHTML = getDtCrewQtyHTML(existingData);
      ...
      case 2041:
        qtySection.innerHTML = getKmKxClipQtyHTML(existingData);
      ...
      case 2031:
      case 2032:
        qtySection.innerHTML = getDtCrewPrepQtyHTML(existingData);
      ...
      case 2013:
        qtySection.innerHTML = getDtCrewJointQtyHTML(existingData, 200, true);
      ...
      case 2003:
      case 2024:
        qtySection.innerHTML = getDtCrewJointQtyHTML(existingData, 300);
        qtySection.addEventListener('input', calcDtCrewJointQtySummary);
        calcDtCrewJointQtySummary();
        break;
      ...
      case 2042:
        qtySection.innerHTML = getKmKxJointQtyHTML(existingData, 300);
        qtySection.addEventListener('input', calcDtCrewJointQtySummary);
        calcDtCrewJointQtySummary();
        break;
      ...
      case 2033:
        qtySection.innerHTML = getDtCrewJointQtyHTML(existingData, 200);
        qtySection.addEventListener('input', calcDtCrewJointQtySummary);
        calcDtCrewJointQtySummary();
        break;
      ...
      case 2025:
        qtySection.innerHTML = getDtCrewEndJointQtyHTML(existingData);
      ...
      case 2026:
      case 2034:
        qtySection.innerHTML = getDtCrewPostQtyHTML(existingData);
      ...
      case 2043:
        qtySection.innerHTML = getKmKxPostQtyHTML(existingData);
      ...
      case 2035:
        qtySection.innerHTML = getInspQtyHTML(existingData, 2, true);
      ...
      case 2015:
      case 2027:
        qtySection.innerHTML = getInspQtyHTML(existingData, 4);
        qtySection.addEventListener('input', calcInspQtySummary);
        calcInspQtySummary();
        break;
      ...
      case 2044:
        qtySection.innerHTML = getKmKxInspQtyHTML(existingData);
        qtySection.addEventListener('input', calcInspQtySummary);
        calcInspQtySummary();
        break;
      ...
      default:
        if (curProc === '조인트') {
          qtySection.innerHTML = getJointQtyHTML(existingData);
          qtySection.addEventListener('input', calcJointQtySummary);
          calcJointQtySummary();
        } else if (curProc === '후가공') {
          qtySection.innerHTML = getPostQtyHTML(existingData);
          qtySection.addEventListener('input', calcPostQtySummary);
          calcPostQtySummary();
        } else {
          qtySection.innerHTML = getStandardQtyHTML(existingData);
          ...
          <table style="width: 100%; border-collapse: collapse; border: 2px solid #000; text-align: center; font-size: 11px; background: #fff; font-family: 'Noto Sans KR', sans-serif;">
            <thead>
              <tr style="background: #fffde7; font-weight: 700;">
                ...
                <th style="border: 1px solid #000; padding: 8px 4px; font-size: 13px; color: #000; width: 70%;">Frunk</th>
              </tr>
            </thead>
            <tbody>
              <tr>
  ...
  const processStandardSave = (targetStatus) => {
    try {
      ...
      if (!curProcessValue || !curProcessValue.value) {
        window.showToast('⚠️ 생산공정을 선택해 주세요.', 'warning');
        return;
      }
      ...
      if (existingData) {
        ...
        if (targetStatus === '임시저장') {
          ...
          window.showToast('✅ 작업일보가 성공적으로 등록 완료되었습니다.', 'success');
        }
      } else {
        store.addReport(reportData);
        if (targetStatus === '임시저장') {
