...
function setupStandardMobileEvents(container, existingData, defaultMakerName, defaultCarCode, loggedInWorkerName = '', materialLots = {}) {
  ...
  // 4자리 양식 고유번호 계산 및 렌더링 함수 (React 하이브리드 파이프라인 연동)
  ...
  const processStandardSave = (targetStatus) => {
    try {
      ...
      const dimData = {
        ...
        ptg_act_초: container.querySelector('#dim_ptg_act_초')?.value || '',
        ptg_act_중: container.querySelector('#dim_ptg_act_중')?.value || '',
        ptg_act_종: container.querySelector('#dim_ptg_act_종')?.value || '',
      ...
      const qtyTable = {
        ...
        ext_short_FL: container.querySelector('#def_ext_short_FL')?.value || '',
        ext_short_FR: container.querySelector('#def_ext_short_FR')?.value || '',
        ext_short_RL: container.querySelector('#def_ext_short_RL')?.value || '',
        ext_short_RR: container.querySelector('#def_ext_short_RR')?.value || '',
        ext_cross_FL: container.querySelector('#def_ext_cross_FL')?.value || '',
        ext_cross_FR: container.querySelector('#def_ext_cross_FR')?.value || '',
        ext_cross_RL: container.querySelector('#def_ext_cross_RL')?.value || '',
        ext_cross_RR: container.querySelector('#def_ext_cross_RR')?.value || '',
        ext_etc_FL: container.querySelector('#def_ext_etc_FL')?.value || '',
        ext_etc_FR: container.querySelector('#def_ext_etc_FR')?.value || '',
        ext_etc_RL: container.querySelector('#def_ext_etc_RL')?.value || '',
        ext_etc_RR: container.querySelector('#def_ext_etc_RR')?.value || '',
...
export function bindTimeWheelPicker(inputElem, titleText = '시간 선택') {
  if (!inputElem) return;
