...
parts.forEach(part => {
  types.forEach((type, idx) => {
    ...
    for (let i = 1; i <= 3; i++) {
      ...
      rowsHTML += `                  <td style="border: 1px solid #000; border-top: ${idx === 0 ? '1px solid #000' : '1px dotted #000'}; ${borderStyle} padding: 2px;"><input type="text" id="${inputId}" class="form-control" style="width:100%; border:none; text-align:center; background:transparent;" value="\${inj['${inputId}'] || ''}" /></td>\n`;
    }
    rowsHTML += '                </tr>\n';
  });
});
...
if (startIndex !== -1) {
  let endIndex = t.indexOf('        return;', startIndex);
  if (endIndex !== -1) {
    endIndex += '        return;\n      }'.length;
    t = t.substring(0, startIndex) + replacementHTML.trim() + '\\n' + t.substring(endIndex);
