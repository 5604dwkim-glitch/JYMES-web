                ...
                <tr style="background: #fffde7; font-weight: 700; color: #000;">
                  ...
                  <th colspan="4" style="border: 1px solid #000; padding: 6px; font-size: 13px;">
                    <label style="margin-right: 20px; cursor: pointer;">
                      <input type="radio" name="sideRadio\${suffix}" value="LH" onchange="document.getElementById('vulc_side\${suffix}').value=this.value" \${val==='LH'?'checked':''} style="margin-right: 4px;"> LH
                    </label>
                    <label style="cursor: pointer;">
                      <input type="radio" name="sideRadio\${suffix}" value="RH" onchange="document.getElementById('vulc_side\${suffix}').value=this.value" \${val==='RH'?'checked':''} style="margin-right: 4px;"> RH
                    </label>
                    <input type="hidden" id="vulc_side\${suffix}" class="vulc-input-dynamic" value="\${val}" />
                  </th>
                </tr>
                <tr style="background: #fffde7; font-weight: 700; color: #000;">
...
let replacement = tpl.trim();
// Fix escaping for the final output JS: replace \\\` with \`
replacement = replacement.replace(/\\\\\`/g, '`');

fs.writeFileSync('src/components/DynamicForms/sections/Section5Renderer.js', before + replacement + '\n\n    ' + after);
console.log('Successfully updated 3002 block with dual table!');
