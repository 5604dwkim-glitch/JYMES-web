...
if (content.includes('formCode === 4032')) {
  ...
  // It's encoded in the file! I will use regex or match lines manually
  ...
  if (targetIdx !== -1) {
    // lines[targetIdx] is the 소재준비 line
    // lines[targetIdx - 1] is `      }`
    // lines[targetIdx - 2] is `        });`
    
    // We replace lines[targetIdx - 1] with `      } else if (formCode === 4032) { ... }`
    const insertCode = `      } else if (formCode === 4032) {
        const v = existingData && existingData.vulcTable ? existingData.vulcTable : {};
        const inp = (prefix, suffix, placeholder = '') => {
          const id = \`vulc_\${prefix}\${suffix}\`;
          const vval = v[\`\${prefix}\${suffix}\`] || '';
    ...
    lines[targetIdx - 1] = insertCode;
    fs.writeFileSync(filePath, lines.join('\\n'), 'utf8');
    console.log('Successfully injected 4032');
  } else {
