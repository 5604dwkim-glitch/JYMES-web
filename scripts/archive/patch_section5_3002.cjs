...
const tpl = `
    if (formCode === 3002) {
      ...
      const sel = (idSuffix) => {
        const id = \`vulc_side_\${idSuffix}\`;
        const val = v[\`side_\${idSuffix}\`] || '';
        return \`
          <select id="\${id}" class="form-control vulc-input-dynamic" style="width:100%; height:20px; font-size:10px; margin-top:2px; padding:0; text-align:center;">
            <option value="" \${val===''?'selected':''}></option>
            <option value="LH" \${val==='LH'?'selected':''}>LH</option>
      ...
      const inp = (prefix) => {
        ...
        const val = v[prefix] || '';
        return \`<input type="text" id="\${id}" class="form-control vulc-input-dynamic" style="width: 70%; height: 24px; text-align: center; font-size: 11px; padding: 2px;" value="\${val}" />\`;
      };
          ...
          <div style="overflow-x: auto;">
            <table style="width: 100%; border-collapse: collapse; border: 2px solid #000; text-align: center; font-size: 11px; background: #fff; font-family: 'Noto Sans KR', sans-serif;">
              ...
              <tbody>
                ...
                <tr>
                  <td style="border: 1px solid #000; background: #fffde7; font-weight: 700; padding: 2px;">
                    초물(Start)<br>\${sel('start_temp')}
                  </td>
...
code = code.replace("if (formCode === 4001) {", tpl + "\n    if (formCode === 4001) {");
fs.writeFileSync('src/components/DynamicForms/sections/Section5Renderer.js', code);
console.log("Section5Renderer.js patched for 3002 Vulcanization table!");
